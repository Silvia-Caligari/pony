...
void LuoRudy::computeQpProperties()
{
    ...
    _I_ion[_qp] = i_Na + i_si + i_K + i_K1 + i_KP + i_b;
    
    
    //update Calcium dynamics

    _Ca_i[_qp] = update_Ca(_Ca_i_old[_qp], _dt, E_si);
...
inline Real LuoRudy::update_m(Real V, Real m_old, Real dt){
    ...
    /*if (_explicit){
    //Explicit Euler
      Real pm = 1 - dt*(alpha_m + beta_m);
      m = pm * m_old + dt * alpha_m;
    }
    
    else{
        //Implicit Euler
     pm = 1 + dt*(alpha_m + beta_m);
     qm = m_old + dt * alpha_m;
     m = qm/pm;
    }*/
...
inline Real LuoRudy::update_h(Real V, Real h_old, Real dt){
    ...
    beta_h = (1.0/(0.13 * (1.0 + std::exp((V + 10.66)/(-11.1))))) * (V >= -40.0) + (3.56 * std::exp(0.079 * V) + 3.1e5 * std::exp(0.35 * V)) * (V < -40.0);
    ...
    /*if (_explicit){
    //Explicit Euler
      Real ph = 1 - dt*(alpha_h + beta_h);
      h = ph * h_old + dt * alpha_h;
    }
    
    else{
        //Implicit Euler
     ph = 1 + dt*(alpha_h + beta_h);
     qh = h_old + dt * alpha_h;
     h = qh/ph;
    }*/
...
inline Real LuoRudy::update_j(Real V, Real j_old, Real dt){
    ...
    /*if (_explicit){
    //Explicit Euler
      Real pj = 1 - dt*(alpha_j + beta_j);
      j = pj * j_old + dt * alpha_j;
    }
    
    else{
        //Implicit Euler
     pj = 1 + dt*(alpha_j + beta_j);
     qj = j_old + dt * alpha_j;
     j = qj/pj;
    }*/
...
inline Real LuoRudy::update_d(Real V, Real d_old, Real dt){
    ...
    /*if (_explicit){
    //Explicit Euler
      Real pd = 1 - dt*(alpha_d + beta_d);
      d = pd * d_old + dt * alpha_d;
    }
    
    else{
        //Implicit Euler
     pd = 1 + dt*(alpha_d + beta_d);
     qd = d_old + dt * alpha_d;
     d = qd/pd;
    }*/
...
inline Real LuoRudy::update_f(Real V, Real f_old, Real dt){
    ...
    /*if (_explicit){
    //Explicit Euler
      Real pf = 1 - dt*(alpha_f + beta_f);
      f = pf * f_old + dt * alpha_f;
    }
    
    else{
        //Implicit Euler
     pf = 1 + dt*(alpha_f + beta_f);
     qf = f_old + dt * alpha_f;
     f = qf/pf;
    }*/
...
inline Real LuoRudy::update_x(Real V, Real x_old, Real dt){
    ...
    /*if (_explicit){
    //Explicit Euler
      Real px = 1 - dt*(alpha_x + beta_x);
      x = px * x_old + dt * alpha_x;
    }
    
    else{
        //Implicit Euler
     px = 1 + dt*(alpha_x + beta_x);
     qx = x_old + dt * alpha_x;
     x = qx/px;
    }*/
...
inline Real LuoRudy::update_Ca(Real Ca_old, Real dt, Real I_s){
    ...
    Real pCai = (1/dt) + 0.07;
    Real qCai = (1/dt) * Ca_old + 0.07e-4 - I_s * 1e-4;
    Ca = qCai/pCai;
